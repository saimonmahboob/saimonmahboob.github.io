...
function Hero({ headline, flare = 'none', perWordRise = false, underline = 'marker', intro = 'off' }) {
  ...
  const renderWord = (w, key, extraProps) => {
    ...
    return (
      <span key={key} className="hf-word" style={{ animationDelay: `${i * 70 + 80}ms` }} {...(extraProps || {})}>
        {w}{'\u00A0'}
      </span>
    );
  };
  ...
  const renderItalicWord = (w) => {
    ...
    if (perWordRise) style.animationDelay = `${i * 70 + 80}ms`;
    ...
    <section id="top" ref={wrapRef} style={heroStyles.wrap} className={sectionClasses.join(' ')}>
      ...
      <style>{`
        ...
        .hero-showcase-back {
          ...
          white-space: nowrap;
          opacity: 0;
          will-change: top, left, transform, color, font-size;
        }
        .hero-showcase-name {
        ...
        .hero-showcase-word {
          ...
          text-align: center;
          white-space: nowrap;
          animation: manifesto-word 300ms cubic-bezier(.18,.78,.22,1) both;
        }
        ...
        .hero-manifesto-word {
          ...
          line-height: 0.95;
          text-align: center;
          animation: manifesto-word 320ms cubic-bezier(.18,.78,.22,1) both;
          white-space: nowrap;
        }
        ...
        @keyframes manifesto-word {
          ...
          40%  { opacity: 1; transform: translateY(-4px) scale(1.04); filter: blur(0); }
        ...
        .hf-rise .hf-last {
          ...
          animation: hf-rise-in 720ms cubic-bezier(.2,.7,.2,1) both;
        }
